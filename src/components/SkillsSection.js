...
const skills = [
  {
    ...
    description:
      "HTML5 is the latest evolution of the standard that defines HTML (HyperText Markup Language). It introduces new features that allow developers to create more engaging and interactive web pages, such as multimedia support for audio and video without plugins, and new elements like <article>, <section>, and <canvas> to structure content more semantically and efficiently. HTML5 is foundational for the structure of the web, ensuring content is properly formatted and accessible across platforms and devices.",
  ...
  },
  {
    ...
    description:
      "TailwindCSS is a highly customizable, utility-first CSS framework that allows developers to build modern, responsive interfaces without leaving the HTML. Instead of writing custom CSS, Tailwind offers pre-built utility classes that handle common design patterns like spacing, typography, and layout, allowing rapid development. It promotes a mobile-first approach and is fully responsive, with features like Flexbox, Grid, and CSS transitions built-in, streamlining the design process and improving productivity.",
...
const SkillsSection = () => {
  ...
  return (
    <section className="mx-auto skills px-12 py-6 flex justify-center items-center w-full lg:mb-6">
      <div className="w-full text-center flex flex-col justify-center items-center">
        <h2
          className="w-[90dvw] lg:w-[70dvw] text-2xl lg:text-3xl text-rws-smoke font-bold bg-rws-dark-blue rounded-full py-2 mb-12"
          style={{
        ...
        <ul
          className={`w-[85dvw] lg:w-[70dvw] flex items-center gap-2 lg:gap-8 justify-center flex-row flex-wrap mt-4 mb-2 lg:mt-8 lg:mb-4 ${
            focusedSkill !== null ? "blur-sm" : ""
          }`}
        >
          ...
          <div className="h-[70dvh] lg:h-[46dvh] bg-rws-smoke w-[90dvw] lg:w-[50dvw] p-6 rounded-3xl shadow-lg transform transition-transform duration-500 text-center mx-4 overflow-auto">
            <div className="flex flex-col justify-center items-center">
              <div className="flex flex-row justify-center items-center mb-4">
                <img
                  src={skills[focusedSkill].imgSrc}
                  alt={skills[focusedSkill].name}
                  className="w-14 mr-2"
                />
                <h3 className="text-3xl font-semibold">
                  {skills[focusedSkill].name}
                </h3>
              </div>
              <p className="text-rws-gray text-lg overflow-y-auto h-[45dvh] lg:h-[25dvh] w-[90%] border border-gray-400 p-2 rounded-lg">
                {skills[focusedSkill].description}
              </p>
            </div>
            <div className="absolute top-6 lg:top-4 right-6 text-xl">
              <p
            ...
            <button
              ...
              className="text-gray-500 absolute bottom-1/2 translate-y-1/2 left-1.5 lg:left-6 text-2xl px-2 cursor-pointer hover:bg-rws-dark-blue hover:text-white rounded-full transition-all duration-200"
            >
              &lt;
            </button>
            <button
              onClick={handleNextSkill}
              className="text-gray-500 absolute bottom-1/2 translate-y-1/2 right-1.5 lg:right-6 text-2xl px-2 cursor-pointer hover:bg-rws-dark-blue hover:text-white rounded-full transition-all duration-200"
            >
              &gt;
            </button>
          </div>
