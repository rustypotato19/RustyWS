  ...
  const fetchRequestByTicketId = async (ticketId) => {
    try {
      const response = await axios.get(
        `http://rustyws.com:5000/api/request/${ticketId}`
      );
    ...
    <div className="container mx-auto p-8">
      ...
      {filteredRequest ? (
        <div className="bg-white p-4 shadow-md rounded-lg">
          ...
          <p>
            <strong>Ticket ID:</strong> {filteredRequest.ticket_id}
          </p>
          <p>
            <strong>Priority:</strong> {filteredRequest.priority}
          </p>
          <p>
            <strong>Status:</strong> {filteredRequest.status}
          </p>
          <p>
            <strong>Contact Info:</strong> {filteredRequest.contact_info}
          </p>
          <p>
            <strong>Date:</strong>{" "}
            {new Date(filteredRequest.request_date).toLocaleDateString()}
          </p>
          <p>
            <strong>Description:</strong>{" "}
            {filteredRequest.description || "No description provided"}
          </p>
        </div>
      ) : (
        <p className="text-gray-500">
          Enter a ticket ID to search for your request.
        </p>
      )}
    </div>
  );
